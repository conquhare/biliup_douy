...
class DanmakuClient(IDanmakuClient):
    ...
    def __init__(self, url, file_name, content=None):
        ...
        self.__danmaku_types: Optional[list] = None  # 弹幕类型筛选
        self.__initial_danmaku_buffer: list = []  # 初始弹幕缓冲区（用于开始录制时打印）
        self.__buffer_start_time: float = 0  # 缓冲区开始时间
    ...
    async def __print_danmaku(self):
        ...
        while True:
            ...
            self.__buffer_start_time = start_time  # 记录缓冲区开始时间
            fmt_file_name = time.strftime(self.__file_name.encode("unicode-escape").decode()).encode().decode(
                "unicode-escape") + '.xml'
            msg_i = 0
            self.__initial_danmaku_buffer = []  # 清空初始弹幕缓冲区
            initial_buffer_logged = False  # 是否已记录初始弹幕
                ...
                while True:
                    ...
                    if not initial_buffer_logged and msg_time - start_time <= 5:
                        ...
                        if self.__initial_danmaku_buffer:
                            danmaku_summary = []
                            for dm in self.__initial_danmaku_buffer[:10]:  # 最多显示10条
                                dm_type = dm.get('msg_type', 'unknown')
                                dm_name = dm.get('name', '未知用户')
                                dm_content = dm.get('content', '')
                                if len(dm_content) > 20:
                                    dm_content = dm_content[:20] + '...'
                                danmaku_summary.append(f"[{dm_type}]{dm_name}: {dm_content}")
                            logger.info(f"{DanmakuClient.__name__}:{self.__url}: 初始弹幕记录 ({len(self.__initial_danmaku_buffer)}条): " + 
